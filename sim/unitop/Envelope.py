...
import numpy 
from numpy.oldnumeric import Float, reshape, array
...
class PTEnvelope(Stream.Stream_Material):
    ...
    def AdjustOldCase(self, version):
        ...
        if version[0] < 78:        
            ...
            if not hasattr(self, 'thEnvelope'):
                self.thEnvelope = None
            if not hasattr(self, 'phEnvelope'):
                self.phEnvelope = None
            
            for key in self.QualityLines.keys():
                line = self.QualityLines[key]
                if line.results:
                    if not hasattr(line.results, 'type'):
                        line.results.type = 'PT'
    ...
    def GetObject(self, name):
        ...
        if self.thEnvelope:
            ...
            elif self.thEnvelope.QualityLines.has_key(name):
                return self.thEnvelope.QualityLines[name]
        if self.phEnvelope:
            if self.phEnvelope.name == name:
                return self.phEnvelope
            elif self.phEnvelope.QualityLines.has_key(name):
    ...
    def Solve(self):
        ...
        if self.thEnvelope:
            for line in self.thEnvelope.QualityLines.values():
                pSpec = line.vapFrac
